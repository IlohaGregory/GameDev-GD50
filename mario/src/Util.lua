...
-- removes individual poles from the sheet
function GeneratePoles(atlas, tilewidth, tileheight)
    local sheetWidth = atlas:getWidth() / tilewidth
    local counter = 1
    local spritesheet = {}
    for x = 0, sheetWidth - 3 do
        spritesheet[counter] = love.graphics.newQuad(x * tilewidth, 0, tilewidth, tileheight, atlas:getDimensions())
        counter = counter + 1
    end
    return spritesheet
end

-- getting out the flags
function GenerateFlags(atlas, tilewidth, tileheight)
    local spritesheet = GenerateQuads(atlas, tilewidth, tileheight)
    local newspritesheet = {}
    for i = 1, #spritesheet do
        if i % 7 == 0 then
            for x = 0, 2 do
                table.insert(newspritesheet, spritesheet[i + x])
            end
        end
    end
