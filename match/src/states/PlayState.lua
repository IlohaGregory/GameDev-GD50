...
function PlayState:update(dt)
    ...
    if self.canInput then
        -- adding functionality for the mouse
        if love.keyboard.mousecord['active'] then
            if love.keyboard.mousecord['x'] < 496 and love.keyboard.mousecord['x'] > 240 then
                self.boardHighlightX = math.floor((love.keyboard.mousecord['x'] - 240) / 32)
            end
            if love.keyboard.mousecord['y'] < 272 and love.keyboard.mousecord['y'] > 16 then
                self.boardHighlightY = math.floor((love.keyboard.mousecord['y'] - 16) / 32)
            end
        end
        ...
        if love.keyboard.wasPressed('return') or love.keyboard.mousecord['enter'] then
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for l, tile in pairs(match) do
                self.score = self.score + 50 + tile.variety * 5
            end
            -- incremening the timer by 1 sec for every tile in a match made
