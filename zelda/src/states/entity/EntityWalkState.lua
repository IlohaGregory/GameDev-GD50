...
-- preventing entities from walking over solid objects
function SolidObjectsCollission(object, entity)
    -- amount by which player is shifted so collision reads false immediately after it reads true
    local padding = 0.1
    -- depending on entity direction the appropriate constrains are applied
    if entity.direction == 'left' then
        entity.x = math.min(object.x + object.width + padding, entity.x + 10)
    elseif entity.direction == 'right' then
        entity.x = math.min(object.x - entity.width - padding, entity.x)
    elseif entity.direction == 'up' then
        entity.y = math.max(object.y + 5.5, entity.y)
    else
        entity.y = math.min(object.y - entity.height - padding, entity.y + entity.height)
    end
end
