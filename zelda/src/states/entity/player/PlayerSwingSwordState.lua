...
function PlayerSwingSwordState:update(dt)
    ...
    for k, entity in pairs(self.dungeon.currentRoom.entities) do
        if entity:collides(self.swordHitbox) then
            entity:damage(1)
            -- spawn heart randomly when an enemy is killed
            if entity.health == 0 and math.random(20) == 2 then
                local heart = GameObject(
                    GAME_OBJECT_DEFS['heart'],
                    math.random(MAP_RENDER_OFFSET_X + 16, 300), math.random(MAP_RENDER_OFFSET_X + 16, 180)
                )
                -- heart collide func
                heart.onCollide = function()
                    if self.player.health < 6 then
                        ...
                        for k, object in ipairs(self.dungeon.currentRoom.objects) do
                            -- remove only the heart that has added health to the player
                            if object.type == 'heart' and object.collected == true then
